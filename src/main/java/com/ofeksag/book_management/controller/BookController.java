...
• Users with ROLE_USER can retrieve books
• Users with ROLE_ADMIN can perform full CRUD operations

Each endpoint includes predefined request examples in Swagger to help you:
• Provide valid input
• Simulate error scenarios (e.g., invalid data, forbidden fields, duplicates)

To test the API:
1. Authenticate via /auth/login and copy the token
2. Click "Authorize" in Swagger and paste the token (no need to add 'Bearer')
3. Use the example inputs to test both success and failure cases

All error responses follow a consistent format and are documented per endpoint.
"""
)
@RestController
...
public class BookController {
    ...
    @Operation(
            ...
            description = "Returns a list of all books in the system. Request must NOT include a body.",
...
**Request body must match the BookRequestDTO schema**, which includes:
- title (required, not blank)
- author (required, not blank)
- publishedDate (required, format: YYYY-MM-DD)
- isbn (required, must be 13-digit number)

Do **NOT** include:
- id
- any unknown fields

These will result in a 400 Bad Request error.
...
""",
    ...
    @Operation(
            ...
            description = "Updates a book by ID. Request body must match the BookRequestDTO schema.",
            security = @SecurityRequirement(name = "bearerAuth"),
            requestBody = @io.swagger.v3.oas.annotations.parameters.RequestBody(
                    required = true,
                    content = @Content(
    ...
    @Operation(
            ...
            description = "Deletes a book by its ID. No body is allowed. Requires ROLE_ADMIN.",
            security = @SecurityRequirement(name = "bearerAuth")
    )
    @ApiResponses(value = {
